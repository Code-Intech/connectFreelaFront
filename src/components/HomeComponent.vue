<template>
    <div class="">
        <div class="container-fluid text-center">
            <div class="row">
                <div class="col-10">
                    <h2 class="text-start">
                        Você é um freelancer que tem dificuldades para encontrar um serviço?
                    </h2>
                    <h2 class="text-start">
                        Ou para encontrar o profissional certo para o seu serviço?
                    </h2>
                    <br /><br />
                    <h2 class="text-start">
                        O ConnectFreela é o lugar certo para te ajudar nesta busca. O site
                        que conecta você, com freelancer de sua escolha.
                    </h2>
                </div>

                <div class="col-2">
                    <img class="img1" src="../assets/img/img1.png" alt="" />
                </div>
            </div>
        </div>


        <br><br><br>


        <div class="container-fluid text-center">
            <div class="row">
                <div class="col-4">
                    <img src="../assets/img/img2.png" alt="" />
                </div>

                <div class="col-8">
                    <h2 class="">
                        Somos um site novo, que planeja otimizar seu tempo e melhorar sua
                        experiencia na procura de profissionais e serviços. Então venha
                        fazer parte de nossa comunidade, e relance seu negócio.
                    </h2>
                </div>
            </div>
        </div>

        <br><br><br >

        <div class="container text-center">
            <div class="row ">
                <div class="col">
                    <h3>Conheça alguns dos nossos serviços</h3>
                </div>
            </div>
        </div>

        <br><br><br>

        <div class="container-fluid text-center">
            <div class="row row-cols-10">
                <div class="col">
                    <span class="icons"><img src="../assets/img/12.svg" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons img"><img src="../assets/img/31.png" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons"><img src="../assets/img/16.svg" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons"><img src="../assets/img/7.svg" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons"><img src="../assets/img/19.svg" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons"><img src="../assets/img/1.svg" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons"><img src="../assets/img/2.svg" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons img"><img src="../assets/img/30.png" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons"><img src="../assets/img/17.svg" alt="" /></span>
                </div>

                <div class="col">
                    <span class="icons"><img src="../assets/img/5.svg" alt="" /></span>
                </div>
            </div>
        </div>

        <br><br><br>

        <div class="container text-center">
            <div class="row ">
                <div class="col">
                    <h3>Trazemos para você modernidade e inovação ao seu negócio, de maneira mais simples.</h3>
                </div>
            </div>
        </div>





    </div>
</template>

<script>
export default {
    name: "HomeComponent",
};
</script>

<style scoped>
.icons img {
    max-width: 100px;
    max-height: 100px;
}



.img1 {
    max-width: 250px;
}

.tt {
    display: inline-block;
}

.ttt {
    width: 1050px;
}

.tt1 {
    width: 600px;
}
</style>
